...
- Set `training_api_url` to the Apps Script web app URL.
- Run `https://script.google.com/macros/s/AKfycbxKZUHO8KiN6-oawtgTnXJy9yf2OPUT1hpnRgcrnygAB8SzMv3J5EylrhC4_Dgv0_dX/exec?action=init` once, then copy `TRAINING_API_KEY` from the Apps Script SETTINGS sheet.
- Set `training_api_key` to that key.
- Set `training_api_mode` to `jsonp` (default) or `fetch` depending on CORS support.
- Optional: set `training_iframe_url` if you want to embed a full booking UI instead of the API flow.
